...
Form::Form(List<Statement> &&statementList, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::Form)
{
    ui->setupUi(this);
    ...
    int i = 0;
    for(auto it = statementList.begin(); it != statementList.end(); ++it, ++i){
        if(statementList.begin() == List<Statement>::iterator(nullptr))
            return;
        ...
        ui->tableWidget->setItem(i, 3, new QTableWidgetItem(QString::fromStdString((*it).address())));
...
void Form::showTable(List<Statement> &&statementList)
{
